...
- **Idempotent** - re-running on an already-archived thread skips it

## Notes

- Respects 4chan API rate limits via parallel connection cap
- Deleted threads or 404s will show an error and clean up the folder
- Threads that are already archived are skipped (delete the folder to re-archive)
